...
sums = df %>%
  group_by(study.labels, splitv, source.name) %>%
...
for(smpl in unique(df$source.name)){
  tblN <- df[df$source.name%in%smpl,c('study.labels','ESCI.N.bin')]
  for(l in unique(tblN$study.labels)){
    sums$offset_densN[sums$source.name%in%smpl&sums$study.labels%in%l] <- tblN$ESCI.N.bin[tblN$study.labels == l]
  }
}
...
png(filename = paste0(dir.out,"/ML2_SplitViolin_Mean_Ori_01-09-2017.png"),width=2400,height=2177, res=250)
g1
dev.off()
