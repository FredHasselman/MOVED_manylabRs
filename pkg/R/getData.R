...
get.analyses <- function(studies      = NA,
  ...
  if(NROW(ML2.df)>0){
      ...
      if(all(nMin1,nMin2)){
        ...
        if(listIT){
          ...
          if(runGroups[g]!="all"){
            ...
            if(sum(sID)==1){
              SourceInfo <- SourceInfoTable[sID, ]
            } else {
              SourceInfo   <- t(ldply(1:13, function(c){
                tbl_df(data.frame(src = paste(unlist(unique(SourceInfoTable[sID,c]),use.names = FALSE),collapse ="\n")))
              }))
            }
            colnames(SourceInfo) <- c("name","name.GLobal",colnames(SourceInfoTable)[3:13])
            rownames(SourceInfo) <- NULL
          } else {
            SourceInfo <- "Global analysis"
          }
          ...
          outputSource[[g]] <- get.output(key      = ML2.key[s,],
                                          ...
                                          descr    = descr,
                                          group    = runGroups[g],
                                          analysis = c("Global","Primary","Secondary")[tp[cnt]],
          ...
          dataSource[[g]] <- list(
            ...
            analysis.type = c("Global","Primary","Secondary")[tp[cnt]],
            analysis.name = ML2.key$study.analysis[[s]],
            stat.info     = ML2.in,
            stat.data.cleanchain = ML2.id,
            stat.data.raw       = ML2.sr[[g]]$RawDataFilter,
          ...
          if(length(ML2.sr[[g]]$RawDataFilter)>1){
            ...
            raw.df[[g]] <-  cbind.data.frame(df1, analysis.type = c("Global","Primary","Secondary")[tp[cnt]], case.include = case.include)
          } else {
            case.include <- ML2.sr[[g]]$RawDataFilter[[1]]$Included
            df1 <- ML2.sr[[g]]$RawDataFilter[[1]][ ,-which(colnames( ML2.sr[[g]]$RawDataFilter[[1]])=="Included")]
            raw.df[[g]] <-  cbind.data.frame(df1, analysis.type = c("Global","Primary","Secondary")[tp[cnt]], cases.include = case.include)
          }
    ...
    if(outdir$ROBJECTS!=""){
      save(dataSource, file = file.path(rootdir,outdir$ROBJECTS,paste0(ML2.key$study.analysis[[s]],"_",c("Global","Primary","Secondary")[tp[cnt]],".RData")))
    }
...
# get.analyses_by_order
get.analyses_by_order <- function(studies   = NA,
                                  tp        = 1,
                                  Nmin.raw  = 30,
                                  Nmin.cond = 15,
                                  rootdir   = "~/Dropbox/Manylabs2/TestOutput",
                                  indir     = list(RAW.DATA = "RAW.DATA.PRIVATE",
                                                   MASTERKEY = "",
                                                   SOURCEINFO = ""),
                                  outdir    = list(ROBJECTS    = "",
  ...
  if(indir$MASTERKEY==""){
    ...
    ML2.key <- read.xlsx(file.path(rootdir,indir$MASTERKEY,"ML2_masteRkey.xlsx"),"ML2masteRkey")
    disp(paste0("Loaded keytable from disk: ML2_masteRkey.xlsx [",file.path(rootdir,indir$MASTERKEY),"]"), header = "get.analysesbyorder", footer = FALSE)
  }
  ...
  for(s in studs){
    ...
    # Get the correct slate according to infor in ML2.key['study.slate']
    ...
    #
    #   # Loop over groups within study
    #   #ugroup       <- unique(eval(parse(text=toRun$ugroup)))
    #   tp           <- 1
    #   ML2.sr       <- list()
    #   ML2.var      <- list()
    #   outputSource <- list()
        ...
        if(listIT){
          ...
          if(runGroups[g]!="all"){
            ...
            if(sum(sID)==1){
              ...
              SourceInfo   <- t(ldply(1:16, function(c){
                tbl_df(data.frame(src = paste(unlist(unique(SourceInfoTable[sID,c]),use.names = FALSE),collapse ="\n")))
              }))
            }
            colnames(SourceInfo) <- c("name","name.GLobal",colnames(SourceInfoTable)[3:16])
            rownames(SourceInfo) <- NULL
          } else {
            SourceInfo <- "Global analysis"
          }
          ...
          outputSource[[g]] <- get.output(key      = ML2.key[s,],
                                          ...
                                          group    = runGroups[g],
                                          analysis = c("Global","Primary","Secondary")[tp[cnt]],
                                          varEqual = ML2.in$stat.params$var.equal,
                                          test     = test,
                                          ESCI     = ESCI,
                                          test.ConsoleOutput = ConsoleOut,
                                          SourceInfo = SourceInfo,
...
decide.analysis <- function(ML2.key, studies=NA, tp = NA){
  ...
  if(is.null(tp)){tp <- NA}
  if(is.na(studies)){studies <- na.exclude(ML2.key$unique.id)}
